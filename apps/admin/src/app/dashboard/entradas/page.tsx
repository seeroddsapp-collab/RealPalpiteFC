  ...
  return (
    <div>
      <div className="flex items-end justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 mb-1">Entradas</h1>
          <p className="text-slate-400 text-sm">{total} palpite{total !== 1 ? 's' : ''} registrado{total !== 1 ? 's' : ''}</p>
        </div>
                ...
                return (
                  <tr key={e.id} className="hover:bg-stone-50 transition-colors">
                    <td className="px-6 py-3">
                      {user ? (
                        <Link href={`/dashboard/usuarios/${user.id}`} className="font-medium text-slate-800 hover:text-gold-600 transition-colors">
