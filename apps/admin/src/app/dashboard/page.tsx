...
export default async function DashboardPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-1">Dashboard</h1>
      <p className="text-slate-400 dark:text-slate-500 text-sm mb-7">Visão geral da operação</p>
        ...
        {cards.map(c => (
          <div key={c.label} className={`rounded-xl p-5 shadow-sm border ${
            c.accent
              ...
              : 'bg-white dark:bg-navy-900 border-stone-200 dark:border-navy-700'
          }`}>
            <p className={`text-xs font-semibold uppercase tracking-widest mb-2 ${c.accent ? 'text-gold-500' : 'text-slate-400 dark:text-slate-500'}`}>
              {c.label}
            </p>
            <p className={`text-2xl font-bold ${c.accent ? 'text-white' : 'text-slate-800 dark:text-slate-200'}`}>
              {c.value}
            </p>
          </div>
        ))}
      ...
      <div className="bg-white dark:bg-navy-900 rounded-xl border border-stone-200 dark:border-navy-700 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-stone-100 dark:border-navy-700 flex items-center gap-2">
          ...
          <h2 className="font-semibold text-slate-800 dark:text-slate-200">Pools Recentes</h2>
        </div>
