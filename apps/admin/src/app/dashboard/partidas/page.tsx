  ...
  return (
    <div>
      <div className="flex items-end justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 mb-1">Partidas</h1>
          <p className="text-slate-400 text-sm">{total} partida{total !== 1 ? 's' : ''} sincronizada{total !== 1 ? 's' : ''}</p>
        </div>
                ...
                return (
                  <tr key={m.id} className="hover:bg-stone-50 transition-colors">
                    <td className="px-6 py-3">
                      <span className="font-medium text-slate-800">{m.home_team}</span>
                      <span className="text-slate-400 mx-2">×</span>
                      <span className="font-medium text-slate-800">{m.away_team}</span>
                    </td>
                    <td className="px-6 py-3 text-slate-500 text-xs">{champ?.name ?? '—'}</td>
                    <td className="px-6 py-3 text-slate-500 text-xs">{fmtDate(m.kickoff_at)}</td>
                    <td className="px-6 py-3">
